...
termux_step_post_get_source() {
	...
	local f
	for f in $(find "$TERMUX_PKG_BUILDER_DIR/../electron-jumbo-host-tools/cr-patches" -maxdepth 1 -type f -name *.patch | sort); do
	...
	local f
	for f in $(find "$TERMUX_PKG_BUILDER_DIR/../electron-jumbo-host-tools/electron-patches" -maxdepth 1 -type f -name *.patch | sort); do
		echo "Applying patch: $(basename $f)"
		patch --silent -p1 < "$f"
	done
	...
	local f
	for f in $(find "$TERMUX_PKG_BUILDER_DIR/../electron-jumbo-host-tools/jumbo-patches" -maxdepth 1 -type f -name *.patch | sort); do
		echo "Applying patch: $(basename $f)"
		patch --silent -p1 < "$f"
	done
...
termux_step_make_install() {
	cd $TERMUX_PKG_BUILDDIR
	local _install_prefix=$TERMUX_PREFIX/lib/electron-jumbo
