...
TERMUX_PKG_VERSION="11.0.30"
TERMUX_PKG_SRCURL=https://github.com/openjdk/jdk11u/archive/refs/tags/jdk-${TERMUX_PKG_VERSION}-ga.tar.gz
TERMUX_PKG_SHA256=5357c80e529dc131cbd8eb1121fa1e54bae7aaa53c038391ea053281266c1718
...
termux_pkg_auto_update() {
	...
	{
		"query": "query {
			repository(owner: \"openjdk\", name: \"jdk11u\") {
	...
	read -r newest_tag < <(echo "$newest_tags" | grep -Po '11\.\d+\.\d+(?=-ga)' | sort -Vr)
...
termux_step_host_build() {
	...
	JDK_BINURL="https://download.java.net/openjdk/jdk11.0.0.2/ri/openjdk-11.0.0.2_linux-x64.tar.gz"
	JDK_BINARCHIVE="${TERMUX_PKG_CACHEDIR}/openjdk-11.0.0.2_linux-x64.tar.gz"
	JDK_BINSHA256=0ab38c0cf5c6809d29c35393faad0ec45703d55a92ec4abd81b55b14fafb3c2d
...
termux_step_configure() {
	local jdk_ldflags="-L${TERMUX_PREFIX}/lib \
		-Wl,-rpath=$TERMUX_PREFIX/lib/jvm/java-11-openjdk/lib \
...
termux_step_make_install() {
	rm -rf $TERMUX_PREFIX/lib/jvm/java-11-openjdk
...
termux_step_post_make_install() {
	...
	binaries="$(find $TERMUX_PREFIX/lib/jvm/java-11-openjdk/bin -executable -type f | xargs -I{} basename "{}" | xargs echo)"
	manpages="$(find $TERMUX_PREFIX/lib/jvm/java-11-openjdk/man/man1 -name "*.1.gz" | xargs -I{} basename "{}" | xargs echo)"

	local failure=false
	for binary in $binaries; do
		grep -q "lib/jvm/java-11-openjdk/bin/${binary}$" "$TERMUX_PKG_BUILDER_DIR"/openjdk-11.alternatives || {
			echo "ERROR: Missing entry for binary: $binary in openjdk-11.alternatives"
			failure=true
		}
	done
	for manpage in $manpages; do
		grep -q "lib/jvm/java-11-openjdk/man/man1/${manpage}$" "$TERMUX_PKG_BUILDER_DIR"/openjdk-11.alternatives || {
			echo "ERROR: Missing entry for manpage: $manpage in openjdk-11.alternatives"
			failure=true
		}
	done
	if [[ "$failure" = true ]]; then
		termux_error_exit "openjdk-11.alternatives is not up to date, please update it."
	fi
}

termux_step_create_debscripts() {
	# For older versions of openjdk-11, openjdk-17 and openjdk-21, we used to provide different alternatives for each binary and manpage.
	...
	# For older versions
	echo 'if [ "$#" = "3" ] && dpkg --compare-versions "$2" le "11.0.24"; then' > ./preinst
	echo '  echo "Removing older alternatives for openjdk-21, openjdk-17 and openjdk-11"' >> ./preinst
	echo '  echo "This may take a while if mandoc package is installed, please wait..."' >> ./preinst
	echo '  echo "Newer versions of openjdk-21, openjdk-17 and openjdk-11 change how alternatives are handled."' >> ./preinst
