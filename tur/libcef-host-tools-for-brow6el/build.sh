...
TERMUX_PKG_DESCRIPTION="Chromium Embedded Framework (Used by brow6el, Host tools)"
...
termux_step_post_get_source() {
	...
	# Apply patches related to libcef
	local f
	for f in $(find "$TERMUX_PKG_BUILDER_DIR/libcef-patches" -maxdepth 1 -type f -name *.patch | sort); do
		echo "Applying patch: $(basename $f)"
		patch -p1 --silent < "$f"
	done
...
termux_step_make() {
	...
	# ninja -C out/Release${libcef_distrib_suffix} cefsimple cefclient
}
...
termux_step_make_install() {
	...
	cp "${v8_tools[@]/#/out/Release${libcef_distrib_suffix}/$cr_v8_toolchain/}" "$TERMUX_PREFIX/opt/$TERMUX_PKG_NAME/$cr_v8_toolchain/"
	...
	cp "${host_tools[@]/#/out/Release${libcef_distrib_suffix}/host/}" "$TERMUX_PREFIX/opt/$TERMUX_PKG_NAME/host/"
	...
	cp "${normal_files[@]/#/out/Release${libcef_distrib_suffix}/}" "$TERMUX_PREFIX/opt/$TERMUX_PKG_NAME/"

	mkdir -p "$TERMUX_PREFIX/opt/$TERMUX_PKG_NAME/obj/third_party/pdfium/"
	cp "out/Release${libcef_distrib_suffix}/obj/third_party/pdfium/libpdfium.a" \
		"$TERMUX_PREFIX/opt/$TERMUX_PKG_NAME/obj/third_party/pdfium/"
}
