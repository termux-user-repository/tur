...
# `bazel5` has been deprecated since Dec 2024.
...
termux_step_configure() {
	...
	# Wrap HOST_CC to auto-detect source type
	mkdir -p $TERMUX_PKG_TMPDIR/wrap-host-cc-bin
	sed -e "s|@TERMUX_PREFIX@|${TERMUX_PREFIX}|g" \
		-e "s|@COMPILER_CC@|/usr/bin/gcc|g" \
		-e "s|@COMPILER_CXX@|/usr/bin/g++|g" \
		"$TERMUX_PKG_BUILDER_DIR"/wrapper.in \
		> $TERMUX_PKG_TMPDIR/wrap-host-cc-bin/gcc
	chmod +x $TERMUX_PKG_TMPDIR/wrap-host-cc-bin/gcc
	export PATH="$TERMUX_PKG_TMPDIR/wrap-host-cc-bin:$PATH"
...
termux_step_make() {
	...
	env CC=$(command -v gcc) CXX=$(command -v gcc) \
		bazelisk build \
